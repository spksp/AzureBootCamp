...
// Delete all the evnes - in a real solution, this would remove the data from cloud datastore.
// Currently this simply remove all event from the mock array in memory
// this will produce unexpected behavior in a stateless kubernetes cluster. 
app.post('/deleteEvents', (req, res) => {
    if(mockEvents.events.length>2){
        mockEvents.events.splice(2,mockEvents.events.length-1);
    }
    res.json(mockEvents);
});
...
// Currently this simply remove all event from the mock array in memory
// this will produce unexpected behavior in a stateless kubernetes cluster. 
app.post('/deleteRecentEvent', (req, res) => {
    mockEvents.events.pop();
    res.json(mockEvents);
});
